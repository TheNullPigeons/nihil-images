...
# Determine OS type and install dependencies
...
if test -f "/tmp/$SLIVER_SERVER"; then
    echo "Moving the Sliver server executable to /usr/local/bin/sliver-server..."
    mv "/tmp/$SLIVER_SERVER" /usr/local/bin/sliver-server
...
if test -f "/tmp/$SLIVER_CLIENT"; then
    echo "Moving the Sliver client executable to /usr/local/bin/sliver-client..."
    mv "/tmp/$SLIVER_CLIENT" /usr/local/bin/sliver-client
